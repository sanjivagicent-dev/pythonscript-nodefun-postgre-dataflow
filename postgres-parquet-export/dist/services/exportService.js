...
export const exportData = async (client) => {
  ...
  `);
    ...
    logger.info({ startDate: startDate.format('YYYY-MM-DD'), endDate: endDate.format('YYYY-MM-DD') }, 'Detected export date range');
    ...
    while (currentStart.isBefore(endDate)) {
        ...
        const data = await client.query(`SELECT * FROM well_data WHERE date >= $1 AND date < $2`, [currentStart.format('YYYY-MM-DD'), chunkEnd.format('YYYY-MM-DD')]);
        ...
        logger.info({ source: filePath, destination: destinationPath }, 'File moved to temporary directory');
